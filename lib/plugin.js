/*! OmniExport Amplenote Plugin. MIT License. Paul Strong(lapluviosilla) */
  ...
  appOption: {
    "Bulk Export": {
      ...
      async run(app) {
        ...
        // File streaming only works on electron
        ...
        if (action === "full-export") {
          ...
          // If user didn't give a query that would return the whole notebook.
          // Warn the user to use the button meant for that purpose to confirm that's what they want to do.
          if (Object.keys(filterParams).length === 0) {
            app.alert(
              "You didn't specify a query! If you meant to export your whole notebook then please use the 'Export Whole Notebook' button"
            );
            return;
          }
  ...
  replaceText: {
    "Export Table as CSV": {
      ...
      async run(app, text) {
        ...
        await exporter.saveTableCSVIfTable();
        return false;
      },
    },
  },
