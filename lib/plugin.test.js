...
describe("Plugin - appOption: Bulk Export", () => {
  ...
  it("should read the query parameters for bulk export", async () => {
    ...
    const bulkExporterSpy = vi.spyOn(BulkExporterModule, "BulkExporter"); // We need to spy on the exporter arguments
    ...
    expect(bulkExporterSpy).toHaveBeenCalledWith(app, [note], "md", expect.any(Object)); // Verify that the second note got filtered out
    bulkExporterSpy.mockRestore();
  });
  it("should export by export tag and remove the tag", async () => {
    ...
    expect(bulkExporterSpy).toHaveBeenCalledWith(
      app,
