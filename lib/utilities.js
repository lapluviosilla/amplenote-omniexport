...
import { loadConflux } from "./dependency_loader";
...
// File Streaming
// ================
export async function startConfluxStream(fileName = "export.zip") {
  if (!confluxWriter) ({ Writer: confluxWriter } = await loadConflux());
...
export async function loadImageAsDataURL(url, returnGifAsStillImage = false) {
  try {
    ...
    if (returnGifAsStillImage && blob.type === "image/gif") {
      ...
      const dataUrl = await gifToStillImage(imageObjectURL, "dataurl");
      URL.revokeObjectURL(imageObjectURL); // Release memory
      return dataUrl;
    } else {
...
/**
 * Converts a gift to a still image
 * @param {String|ObjectURL} url dataurl or object url
 * @param {String} type "blob" or "dataurl"
 * @returns {Promise<}
 */
export function gifToStillImage(url, type = "dataurl") {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (type === "blob") {
        canvasToBlobAsync(canvas, "image/png").then((blob) => resolve(blob));
...
/**
 * Wrapper function for gifToStillImage that allows us to pass in a blob
 * @param {Blob} blob
 * @param {String} type optional type indicating blob or dataurl return
 * @returns {Blob|String} still image
 */
export async function gifBlobToStillImage(blob, type = "blob") {
  const imageObjectURL = URL.createObjectURL(blob);
  // Create an image element to draw the first frame of the GIF
  const stillImage = await gifToStillImage(imageObjectURL, type);
  URL.revokeObjectURL(imageObjectURL); // Release memory
  return stillImage;
}
...
export function proxifyAsset(url) {
  if (isAttachmentURL(url)) {
    return proxifyAttachment(url);
  } else {
 ...
 */
export async function proxifyAttachment(url) {
  if (isAttachmentURL(url)) {
    const assetId = url.split("/").pop();
    const attachmentUrl = await getAppInterface().getAttachmentURL(assetId);
    if (!attachmentUrl) return url; // If attachment is no longer valid then return as is
...
export function proxifyImage(url) {
  // Check if it's a valid base64 image URL (data URI)
  const base64Pattern = /^data:image\/(png|jpeg|jpg|gif|webp);base64,/;
  if (base64Pattern.test(url)) {
    return url; // Return base64 as is
  }

  // Check if it's a valid relative file reference (local file)
  const localFilePattern = /^(?:\.\/|\.\.\/|[^\/\\]+\w+\/?)*\w+\.\w+$/;
  if (localFilePattern.test(url)) {
    return url; // Return local file path as is
  }
...
/**
 *
 * @param {String} attachmentUrl - attachment://uuid
 * @returns {Promise<Response|null>}
 */
export async function loadAttachment(attachmentUrl) {
  try {
    const fetchURL = await proxifyAttachment(attachmentUrl);
    ...
    if (!response.ok) {
      console.warn(
        `Bulk Export - Failed to fetch attachment: ${response.status} ${response.statusText}`
      );
      return null;
    }
    ...
    console.warn(`Bulk Export - Failed to load attachment: ${error.message}`);
    return null;
  }
}

export function isAttachmentURL(href) {
 ...
 */
function canvasToBlobAsync(canvas, type = "image/png", quality = 1) {
  return new Promise((resolve, reject) => {
    canvas.toBlob(
      (blob) => {
        if (blob) {
...
export function escapeBackslashNewlines(markdown) {
  ...
  return markdown.replace(doubleBackslashLinePattern, "\n");
}
...
/**
 * Escapes a URL or relative file path for safe use in Markdown links by percent-encoding special characters.
 * @param {string} url - The original URL or relative file path to be escaped.
 * @returns {string} - The escaped URL or file path.
 */
export function escapeUrl(url) {
  try {
    return encodeURI(url);
  } catch (e) {
    console.error("Invalid URL or path provided:", url);
    return url;
  }
}
