      ...
      case "ordered_list_open":
        {
          ...
          while (listNesting > 0 && tokenIndex < tokens.length) {
            ...
            if (currentToken.type === "list_item_open") {
              ...
              // TODO: Change bullet types based on nesting
 ...
 * Currently unused, but may be necessary later for more advanced processing
 *
      ...
      case "image":
        {
          ...
          const widthAttr = token.attrs.find((attr) => attr[0] === "width");
          const width = widthAttr ? widthAttr[1] : 250; // Default to 250 width
          const alt = altAttr ? altAttr[1] : "";
          // const imageNameParts = token.content.split("|"); /--- Now handled in asset exporter
          ...
          let imageKey;
          const constrainedWidth = width > pageContentWidth ? pageContentWidth : width;

          // We actually handle this in the asset exporter, but leaving for redundancy in case we use this elsewhere
          // If a http or data url was left in then we want to extract them.
          if (isValidHttpUrl(src) || /^data:image\/(png|jpeg|jpg|gif|webp);base64,/.test(src)) {
            // If it references a url, then we need to put in the images array
            // Generate a unique key for the image
            imageKey = Object.keys(images).find((key) => images[key] === src);
          ...
          content.push({
            image: imageKey || src,
            width: constrainedWidth, // Adjust as needed
            alt,
          });
        }
        break;
