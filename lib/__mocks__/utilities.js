...
import TestImageDataUrl from "../../images/test.png";
import TestGifDataUrl from "../../images/test.gif";

const TestImage = (await fetch(TestImageDataUrl)).blob();
const TestGif = (await fetch(TestGifDataUrl)).blob();
...
export const loadImage = vi.fn(async (imageUrl) => {
  if (imageUrl === "test.gif") {
    return new Response(TestGif, { headers: { "Content-Type": "image/gif" } });
  } else {
    return new Response(TestImage, { headers: { "Content-Type": "image/png" } });
  }
});
...
export const loadAttachmentAsBlob = vi.fn(async (url) => {
  return new Blob(["attachment data"], { headers: { "Content-Type": "application/pdf" } });
});
export const loadAttachment = vi.fn(async (url) => {
  return new Response(
    new Blob(["attachment data"], { headers: { "Content-Type": "application/pdf" } })
  );
});
export const gifBlobToStillImage = vi.fn(async (blob, type) => {
  return loadImageAsBlob();
});
