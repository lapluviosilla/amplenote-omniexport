...
export class MarkdownExporter {
   ...
   * @param {AssetExporter} assetExporter Optionally pass in a asset exporter if you are exporting multiple files
   */
  constructor(title, markdownText, assetExporter = new AssetExporter()) {
  ...
  _setupHTMLRenderer() {
    ...
    //   // const imageNameParts = tokens[idx].content.split("|");

    //   // // Extract the image width from the name "name|width", or default to 250px
    //   // const width =
    //   //   imageNameParts.length > 1 ? parseInt(imageNameParts[imageNameParts.length - 1], 10) : null;
    //   // if (width) {
    //   //   tokens[idx].attrSet("width", width);
    //   //   tokens[idx].content = tokens[idx].children[0].content = imageNameParts
    //   //     .slice(0, imageNameParts.length - 1)
    //   //     .join("|");
    //   // }

    //   // Parse the image width from the name
  ...
  async toMarkdown(singleFile = false) {
  ...
  async toLaTeX() {
    ...
    this.assetExporter.setAssetStrategy("local", "local");
    // TODO: Make it so that we can render gifs as still images for local file strategy (currently only supported in dataurl)
...
\\end{document}
  ...
  async saveLaTeX(fileName = "output.tex") {
    ...
    // const { outLatex, images } = await processLaTeXForImages(latexContent);
