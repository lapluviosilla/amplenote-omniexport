...
export class MarkdownExporter {
  ...
  async _loadHtmlDocx() {
    if (!htmlDocx) await loadHtmlDocx();
  }
  ...
  async _ensureBaseDependenciesLoaded() {
    [markdownit, saveAs] = await Promise.all([loadMarkdownIt(), loadFileSaver()]);
  }
  ...
  async toDOCX(fileName = "output.docx") {
    ...
    const docxContent = htmlDocx.asBlob(this.htmlContent);
