...
import markdownToPDF from "./markdown_pdf_converter";
import { loadImageAsDataURL } from "./utilities";
export class MarkdownExporter {
  ...
  async _loadJsPDF() {
    ...
    // }
    if (!window.pdfmake) {
      await this._loadScript("https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.2.4/pdfmake.min.js");
    }
    if (!window.vfs_fonts) {
      await this._loadScript("https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.2.4/vfs_fonts.js");
    }
  }
  ...
  async toPDF(fileName = "output.pdf") {
    ...
    await Promise.all(
      Object.entries(images).map(async ([k, v]) => {
        images[k] = await loadImageAsDataURL(v);
      })
    );
