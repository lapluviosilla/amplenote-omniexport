...
/** Bulk Exporter
 * options
 * - directory: "tag" - by first tag, "flat" - all in root directory -- not implemented
...
export class BulkExporter {
  ...
  async export() {
    ...
    const assetExporter = new AssetExporter({ exportAttachments: this.options.includeAttachments });
    const exporter = new MarkdownExporter(null, null, assetExporter);
    ...
    let assetStream;
    // Write Attachments
    // TODO: Update progress for attachments by piping through a progress reporter
    ...
    // Wait for the zip piping to finish
    if (pipePromise) {
      await pipePromise;
    } else {
  ...
  /** Export bulk notes as a single epub notebook */
  async exportEpubNotebook() {
    ...
    await exporter.initialize();
    // Epubs don't support attachments, so just load images
  ...
  /** Export notes as CSV (no asset processing) */
  async exportCSV() {
