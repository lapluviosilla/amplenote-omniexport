...
import { processLaTeXForImages } from "./markdown_latex_renderer";
...
export class BulkExporter {
  constructor(app, noteList, format, options = {}) {
    ...
    this.format = format;
    this.options = { ...{ directory: "tag", includeAttachments: false }, ...options };
  }
      ...
      switch (this.format) {
        ...
        case "latex":
          ...
          await writer.write({
            ...
            lastModified: parseISOString(note.updated),
            stream: () =>
              new Response(outLatex, { headers: { "Content-Type": "application/x-tex" } }).body,
          });
  ...
  async exportEpubNotebook() {
    ...
    const chapterLoaders = this.exportNoteList.map(async (note) => {
      ...
      return { title: note.name || "", content: exporter.toHTML(false) };
    });
