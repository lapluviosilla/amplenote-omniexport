...
/*! With modifications by Paul Strong (2024)
...
// Only use stream on electron
// TODO: Re-enable stream on other browsers once plugin api supports it
let useBlobFallback =
...
/** Paul Strong: Modified to use Amplenote renderEmbed as a tunnel relay instead of loading iframe directly */
...
function createWriteStream(filename, options, size) {
  ...
  useBlobFallback = useBlobFallback || opts.useBlob;
  if (!useBlobFallback) {
    loadTransporter();
    ...
    new streamSaver.WritableStream(
      {
        ...
        close() {
          if (useBlobFallback) {
            // Use FileSaver because some browsers have CORS issues
            const blob = new Blob(chunks, { type: "application/octet-stream; charset=utf-8" });
            const app = getAppInterface();
            app.saveFile(blob, filename);
            // loadFileSaver().then(function (saveAs) {
